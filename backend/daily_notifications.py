...
def _post_neal_street_digest(session: Session, week_start: str, today_str: str) -> int:
    ...
    today_people = sorted({
        ...
        if row.date == today_str and row.location == "Neal Street"
    })

    if today_people:
        text = f"🏢 At Neal Street today: {', '.join(today_people)}"
    else:
        text = "🏢 Nobody's logged as being at Neal Street today."

    slack_client.post_message(SLACK_GENERAL_CHANNEL_ID, text)
    return len(today_people)
