...
- **Tomorrow Neal Street digest** — every weekday afternoon (4pm London), a message goes to the same channel listing who's at Neal Street the next working day, in the same day-by-day style as the week summary below. On Fridays, "tomorrow" would be Saturday (not useful), so it instead posts the whole of next week's schedule (same week-summary format, headed "Here's who's at Neal Street next week") — one action, branching by day, rather than a separate job.
- All of the above (today's digest, tomorrow/next-week digest, and the post-submission week summary) show **Neal Street and Client Office** on separate lines, matching the tracker website's Who's Where grouping — Client Office is further broken out by client name (e.g. "Sky: @Alice"). Other locations (WFH, Holiday, etc.) aren't shown, since these messages are specifically answering "who's in an office".
