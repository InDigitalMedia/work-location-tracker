...
- **Daily office digest** — every weekday morning, a message goes to a configured channel: ":coffee: Good morning everyone! Here's who's in the office today :point_down:", followed by the day's Neal Street/Client Office breakdown. Runs as its own scheduled action, independent of the reminder above.
- **Tomorrow/next-week office digest** — every weekday afternoon (4pm London), a message goes to the same channel with who's in tomorrow (":wave: Good afternoon everyone! Here's who will be in the office tomorrow :point_down:"). On Fridays, "tomorrow" would be Saturday (not useful), so it instead posts the whole of next week's schedule (":wave: Good afternoon everyone! Here's who will be in the office next week :point_down:") — one action, branching by day, rather than a separate job.
- All of the above (today's digest, tomorrow/next-week digest, and the post-submission week summary) show **Neal Street and Client Office** on separate lines, each with a headcount (e.g. "🏢 Neal Street (5)"), matching the tracker website's Who's Where grouping — Client Office is further broken out by client name (e.g. "Sky: @Alice"). Other locations (WFH, Holiday, etc.) aren't shown, since these messages are specifically answering "who's in an office". Header wording says "in the office" generically; the Neal Street/Client Office labels stay as-is within the breakdown itself.
...
- **Week summary** — after successfully saving (via the modal or "Same as last week"), you privately get an Officely-style breakdown ("Here's who's in the office this week") of who's at Neal Street/Client Office each day that week, with a "See Full Schedule" link back to the web app.
...
- Scheduling: `.github/workflows/slack-daily-digest.yml` and `.github/workflows/slack-unfilled-reminders.yml` (both 9am, fired independently), `.github/workflows/slack-tomorrow-digest.yml` (4pm tomorrow digest), and `.github/workflows/slack-next-week-reminder.yml` (Friday 2pm next-week reminder) — each a GitHub Actions cron firing at **both** UTC-equivalents of the target London hour (08:00/09:00, 15:00/16:00, and 13:00/14:00 UTC respectively). Each endpoint checks whether it's actually the target hour in London and no-ops on whichever firing doesn't match — this is deliberate, so the crons never need editing for BST/GMT. All four workflows also support manual "Run workflow" with a `force` checkbox to bypass the gate entirely.
...
| `SLACK_GENERAL_CHANNEL_ID` | Which channel gets the daily Neal Street digests (today's at 9am, tomorrow's at 4pm) | Plain value in `render.yaml`, currently `CGHDKEF6J` |
