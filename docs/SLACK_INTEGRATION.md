...
- Backend: `backend/slack_routes.py` (the 3 HTTP endpoints), `backend/slack_views.py` (Block Kit message/modal building + parsing), `backend/slack_client.py` (thin Slack Web API wrapper + request signature verification), `backend/slack_directory.py` (matches `team-members.json` names against the Slack workspace directory), `backend/daily_notifications.py` (the scheduled job's logic).
- The 3 endpoints, all under `app.include_router(slack_router)` in `backend/app.py`:
  ...
  - `POST /internal/slack/daily-notifications` — the scheduler trigger (gated by its own secret, see below)
- Scheduling: `.github/workflows/slack-daily.yml`, a GitHub Actions cron firing at **both** 08:00 and 09:00 UTC every weekday. The endpoint itself checks whether it's actually 9am London time and no-ops on whichever firing doesn't match — this is deliberate, so the cron never needs editing for BST/GMT.
...
| `SLACK_GENERAL_CHANNEL_ID` | Which channel gets the daily Neal Street digest | Plain value in `render.yaml`, currently `C0BJV5KDT4P` |
